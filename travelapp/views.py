...
def FlightView(request):
    ...
    if request.method=="POST":
        if form.is_valid():
            ...
            form = {'form': form}
            response = {**f,**d,**form}
            return render(request,'flight.html',response)
        else:
...
def Dashboard(request):
    f1 = BookFlight.objects.filter()
    h1 = BookHotel.objects.filter()
    p1 = BookPackage.objects.filter()
    f={'flights':f1}
    h={'hotels':h1}
    p={'packages':p1}
    response= {**f,**h,**p}
...
def feedback(request):
    return redirect('https://docs.google.com/forms/d/1GsZGjsLrIdIlhEP0-QUC7Qq1LHoz0EZqsj-idGHZbHY/viewform?usp=sf_link')
